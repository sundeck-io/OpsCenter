...
def test_basic_probe(session):
    ...
    assert _expected_condition_verification_sql(p) in session._sql[0].lower(), 'Unexpected probe condition query'


@pytest.mark.parametrize("method", ["EMAIL", NotificationMethod.EMAIL, "SLACK", NotificationMethod.SLACK])
