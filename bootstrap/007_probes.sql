...
BEGIN
    -- Add NOTIFY_WRITER_METHOD column
    IF (NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'INTERNAL' AND TABLE_NAME = 'PREDEFINED_PROBES' AND COLUMN_NAME = 'NOTIFY_WRITER_METHOD')) THEN
        ALTER TABLE IF EXISTS INTERNAL.PREDEFINED_PROBES ADD COLUMN NOTIFY_WRITER_METHOD STRING DEFAULT 'EMAIL';
    END IF;

    -- Add NOTIFY_OTHER_METHOD column
    IF (NOT EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'INTERNAL' AND TABLE_NAME = 'PREDEFINED_PROBES' AND COLUMN_NAME = 'NOTIFY_OTHER_METHOD')) THEN
        ALTER TABLE IF EXISTS INTERNAL.PREDEFINED_PROBES ADD COLUMN NOTIFY_OTHER_METHOD STRING DEFAULT 'EMAIL';
    END IF;

    -- Rename EMAIL_WRITER to NOTIFY_WRITER
    IF (EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'INTERNAL' AND TABLE_NAME = 'PREDEFINED_PROBES' AND COLUMN_NAME = 'EMAIL_WRITER')) THEN
        ALTER TABLE IF EXISTS INTERNAL.PREDEFINED_PROBES RENAME COLUMN EMAIL_WRITER to NOTIFY_WRITER;
    END IF;

    -- Rename EMAIL_OTHER to NOTIFY_OTHER
    IF (EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'INTERNAL' AND TABLE_NAME = 'PREDEFINED_PROBES' AND COLUMN_NAME = 'EMAIL_OTHER')) THEN
        ALTER TABLE IF EXISTS INTERNAL.PREDEFINED_PROBES RENAME COLUMN EMAIL_OTHER to NOTIFY_OTHER;
    END IF;
